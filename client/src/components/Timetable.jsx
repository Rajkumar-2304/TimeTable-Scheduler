...
export default function Timetable() {
  ...
  async function exportPDF() {
    ...
    try {
      ...
      const institutionName = config?.institution || 'Institution';
      pdf.setFontSize(16);
      pdf.setTextColor(15, 23, 42);
      pdf.setFont('helvetica', 'bold');
      pdf.text(institutionName, pW / 2, margin + 14, { align: 'center' });

      // ── Sub-header: View label + selected name ──
      const viewLabel = view === 'group' ? 'Class / Group' : view === 'faculty' ? 'Faculty' : 'Room';
      const selName   = selectedItem?.name || '';
      pdf.setFontSize(10);
      pdf.setTextColor(71, 85, 105);
      pdf.setFont('helvetica', 'normal');
      pdf.text(`${viewLabel}: ${selName}`, pW / 2, margin + 28, { align: 'center' });

      // ── Divider line ──
      pdf.setDrawColor(203, 213, 225);
      pdf.setLineWidth(0.5);
      pdf.line(margin, margin + 36, pW - margin, margin + 36);

      // ── Timetable image ──
      const headerH = margin + 44;                       // top reserved height
      const footerH = 24;                                // bottom reserved for footer
      const availW  = pW - margin * 2;
      const availH  = pH - headerH - footerH - margin;  // usable vertical space
      ...
      pdf.setTextColor(15, 23, 42);
      pdf.setFont('helvetica', 'bold');
      pdf.text(institutionName, p2W / 2, margin + 14, { align: 'center' });

      pdf.setFontSize(11);
      pdf.setTextColor(59, 130, 246);
      pdf.text('Faculty & Subject Assignments', p2W / 2, margin + 28, { align: 'center' });

      pdf.setFontSize(9);
      pdf.setTextColor(71, 85, 105);
      pdf.setFont('helvetica', 'normal');
      pdf.text(`${viewLabel}: ${selName}`, p2W / 2, margin + 42, { align: 'center' });

      pdf.setDrawColor(203, 213, 225);
      pdf.line(margin, margin + 50, p2W - margin, margin + 50);

      // Table setup
      const tableTop = margin + 58;
      const colWidth = (p2W - 2 * margin) / 3;

      // Header row
      pdf.setFillColor(37, 99, 235);
      pdf.rect(margin, tableTop, p2W - 2 * margin, 26, 'F');
      pdf.setFontSize(10);
      pdf.setTextColor(255, 255, 255);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Faculty',            margin + 8,              tableTop + 17);
      pdf.text('Subject(s) Handled', margin + colWidth + 8,   tableTop + 17);
      pdf.text('Department',         margin + 2 * colWidth + 8, tableTop + 17);
      ...
      Object.keys(facultySubjectMap).forEach(facId => {
        ...
        if (fac) {
          const subs = Array.from(facultySubjectMap[facId])
      ...
      facultyList.forEach(item => {
        if (altRow) {
          ...
          pdf.rect(margin, rowY, p2W - 2 * margin, rowH, 'F');
        }
        pdf.setFontSize(9.5);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(30, 41, 59);
        pdf.text(item.fac.name,  margin + 8,              rowY + 15);
        pdf.text(item.subs,      margin + colWidth + 8,   rowY + 15);
        pdf.text(item.fac.dept || '—', margin + 2 * colWidth + 8, rowY + 15);

        // row border
        pdf.setDrawColor(226, 232, 240);
        pdf.line(margin, rowY + rowH, p2W - margin, rowY + rowH);

        rowY   += rowH;
        altRow  = !altRow;

        if (rowY > p2H - margin - 20) {
          pdf.addPage('a4', 'portrait');
          pdf.setFillColor(255, 255, 255);
          pdf.rect(0, 0, p2W, p2H, 'F');
          rowY   = margin + 20;
      ...
      pdf.text(
        `Generated by CRT Scheduler — ${new Date().toLocaleString()}`,
        p2W / 2, p2H - 10, { align: 'center' }
      );

      const safeName = (selectedItem?.name || selId).replace(/[^a-z0-9]/gi, '_');
      pdf.save(`timetable_${view}_${safeName}.pdf`);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="timetable-outer" ref={gridRef}>
        {/* Institution + Group header — visible inside the captured area */}
        <div style={{
          textAlign: 'center',
          ...
          <div style={{ fontSize: '0.82rem', color: 'var(--text-3)', marginTop: 3 }}>
            {view === 'group'   ? 'Class / Group' :
             view === 'faculty' ? 'Faculty'       : 'Room'}
